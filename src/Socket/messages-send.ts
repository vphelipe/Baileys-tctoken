...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const relayMessage = async (
		...
		await authState.keys.transaction(async () => {
			...
			const is1on1Send = !isGroup && !isRetryResend && !isStatus
				&& !areJidsSameUser(destinationJid, meId)
			...
			if (is1on1Send) {
				...
				// Only for LID recipients when nctSalt exists (matches WA Web genCsTokenBody)
				if (!hasTcToken && authState.creds.nctSalt?.length) {
					try {
						const recipientLid = isLidUser(destinationJid) ? destinationJid : null
						if (recipientLid) {
			...
			if (is1on1Send && shouldSendNewTcToken(tcSenderTimestamp)) {
				;(async () => {
